...
export default function AddMoney(props: any) {
  ...
  function Razorpayorder() {
    ...
    if (inputRef.current) {
      ...
      axios
        .post(
          url + "/payment/razorpayorder",
          { user_id: user._id, amount },
        ...
        .then((res) => {
          const options = {
            ...
            handler: function (response: any) {
              axios
                .post(
                  url + "/payment/razorpayverify",
                  { _id: user._id, amount, response: response },
                  { headers: { Authorization: h } }
                )
                .then((res) => {
                  props.f();
      ...
      .then((res) => {
        ...
        cf.checkout(checkoutOptions).then((result: any) => {
          ...
          if (result.paymentDetails) {
            ...
            axios
              .post(
                url + "/payment/cashfreeverify",
                { _id: user._id, amount, order_id: res.data.response.order_id },
                { headers: { Authorization: h } }
              )
              .then(() => {
                props.f();
    ...
    <div className="w-full bg-gray-100 p-2">
      <div className="bg-white shadow-md rounded-lg p-8 h-full w-full">
        ...
        <div className="flex justify-center flex-col sm:flex-row items-center mb-4 gap-7">
          <img
            src={Razorpaylogo}
            onClick={() => setChoice("Razorpay")}
            alt="Razorpay_logo"
            className="w-64 hover:cursor-pointer hover:scale-110 transition-all"
          />
          <img
            src={Cashfree}
            onClick={() => setChoice("CashFree")}
            alt="Cashfree_logo"
            className="w-64 hover:cursor-pointer hover:scale-110 transition-all"
          />
        </div>
        <div className="flex justify-center items-center">
          {(choice === "Razorpay" && (
            <button
              onClick={order}
              className="font-bold bg-blue-800 text-white p-2 rounded-lg"
            >
              Pay with Razorpay
            </button>
          )) ||
