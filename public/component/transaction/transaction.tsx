...
import { baseurl, userinfo } from "../../config/recoil/atom";
...
export default function Transaction() {
  ...
  return (
    <div className="bg-sky-700 min-h-screen w-full overflow-auto">
      <h1 className="font-semibold text-4xl p-5 pl-10 text-white">
        Transaction History
      </h1>
      <div className="flex justify-center w-full">
        {transc.length === 0 ? (
          <div className="text-center text-2xl">No Transactions Yet</div>
        ) : (
...
function TransactionHistory(props: any) {
  ...
  return (
    <div className="bg-white w-full m-2 sm:m-10 rounded-md shadow-md sm:p-5">
      ...
      {props.transc.map((item: any) => {
        return (
          <div className="sm:grid grid-cols-4 gap-4 rounded-sm hover:bg-slate-100 p-5 border-b-2 border-gray-300 w-full">
            <div className="w-fit bg-blue-100 text-blue-800 text-2xl font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">
              <span className="relative sm:hidden">TxID:</span>
              {item.id}
            </div>
            ...
            <div
              className={
                (item.sender === user._id ? "text-red-700" : "text-green-700") +
                " text-xl"
              }
            >
